      ...
      <el-row :gutter="5">
        ...
        <el-col :span="18">
          <el-form :inline="true" style="float:right;">
            <el-form-item>
              <el-input v-model="conditions.search.levelname" placeholder="请输入关键字" @keyup.enter.native="conditionsearch"/>
            </el-form-item>
            <el-form-item>
...
export default {
  ...
  methods: {
    ...
    // 分页抓取数据
    fetchData() {
      getConflevelList(this.conditions).then(response => {
    ...
    handleEdit(index, row) {
      this.conflevelform.id = row.id
      this.conflevelform.levelname = row.levelname
      this.conflevelform.leveldes = row.leveldes
      this.dialogFormVisible = true
    },
    // 删除
    handleDelete(index, row) {
      const rowid = row.id
      this.$confirm('你确定要删除' + row.levelname + '吗?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        removeConflevel(rowid).then(response => {
    ...
    batchremote() {
      ...
      if (delids.length > 0) {
        this.$confirm('你确定要删除这' + delids.length + '条记录吗?', '提示', {
          ...
          batchrmConflevel(delids).then(response => {
            if (response.ok) {
              this.$message.success('删除成功')
              // 重新fetchdata
              this.fetchData()
    ...
    // 确定修改
    suremodify() {
      modifyConflevel(this.conflevelform).then(response => {
        if (response.ok) {
    ...
    // 确定新增
    sureAddSemester() {
      this.conflevelform.id = ''
      newConfalevel(this.conflevelform).then(response => {
        if (response.ok) {
